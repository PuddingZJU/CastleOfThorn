...
CCScene* FirstfloorScene::scene(){
	...
	FirstfloorScene *layer = FirstfloorScene::create();
...
bool FirstfloorScene::init(){
	...
	CCRPGTalkBox* box1 = CCRPGTalkBox::create(12, "dialog-box.png", "Firstfloor.txt", 13, CCSizeMake(visibleSize.width, 200), 1,callfunc_selector(FirstfloorScene::LoadScene),this,screenpos);
	addChild(box1,9999,12);
	box1->NextText();
	return true;
}
void FirstfloorScene::LoadScene(){
	removeChildByTag(12);
	CCSize visibleSize = CCDirector::sharedDirector()->getVisibleSize();
...
void FirstfloorScene::A_Button_Pressed(){
...
void FirstfloorScene::B_Button_Pressed(){
	CCSize visibleSize = CCDirector::sharedDirector()->getVisibleSize();
	player->setPosition(ccp(visibleSize.width/2-32,visibleSize.height/2));
}

void FirstfloorScene::Scan_cur_block(CCPoint pos){
...
void FirstfloorScene::Scan_Face_To_block(CCPoint pos){
